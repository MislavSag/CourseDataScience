...
**Result:** data.table is `r mbx`x faster! More benchmarks can be found [here](https://h2oai.github.io/db-benchmark/){target="_blank"}
...
::: r-stack
DT\[[i]{style="color: #66C2A5;"}, [j]{style="color: #FC8D62;"}, [by]{style="color: #8DA0CB;"}\]
:::

:::::: columns
::: {.column width="33%"}
[On which rows?]{style="color: #66C2A5;"}
:::

::: {.column width="33%"}
[What to do?]{style="color: #FC8D62;"}
:::

::: {.column width="33%"}
[Grouped by what?]{style="color: #8DA0CB;"}
:::
::::::
...
:::::: columns
::: {.column width="33%"}
[filter(); slice(); arrange()]{style="color: #66C2A5;"}
:::

::: {.column width="33%"}
[select(); mutate()]{style="color: #FC8D62;"}
:::

::: {.column width="33%"}
[group_by()]{style="color: #8DA0CB;"}
:::
::::::
...
::: fragment
Note that we don't actually need commas when we're only subsetting on `i` (i.e. no `j` or `by` components). - `DT[x=="string"]` is equivalent to `DT[x=="string", ]` - `DT[1:10]` is equivalent to `DT[1:10, ]` - etc.
:::
...
::: fragment
data.table recognizes that all of these verbs are just different versions of telling R... \>"*Do something to this variable in my dataset*"
...
3.  let form: `DT[, let(var1=val1, var2=val2)]`
:::

::: fragment
Personally, I prefer the LHS form when creating many columns programatically.
...
::: fragment
You can't use variable crated inside let to create new variables.

```{r mod_cols2, error=TRUE}
DT[, let(z = 5:6, z_sq = z^2)][]
```
:::
...
:::{.fragment}
You can also use base pipe operator:
...
More in [blog post](https://www.r-bloggers.com/2024/01/piping-data-tables/){target="_blank"}
...
You can think of it as one of data.table's syntactical quirks. But, really, it's just there to give you more options. You can often — if not always — use these three forms interchangeably in data.table: - `.(var1, var2, ...)` - `list(var1, var2, ...)` - `c("var1", "var2", ...)`
...
::: fragment
data.table's `by` argument functions very similarly to the `dplyr::group_by` equivalent.
...
-   `starwars_dt[, mean(mass, na.rm=T), by = height>190]`: Conditionals.
...
data.table offers its own functions for flexibly reshaping data: - `melt()`: convert wide data to long data - `dcast()`: convert long data to wide data
:::

::: fragment
However, I also want to flag the [**tidyfast**](https://tysonbarrett.com/tidyfast/index.html) package by Tyson Barrett, which implements data.table versions of the tidyr::pivot\* functions (among other things). - `tidyfast::dt_pivot_longer()`: wide to long - `tidyfast::dt_pivot_wider()`: long to wide
:::
...
::: fragment
Hadley Wickham's **dtplyr** package provides a data.table "back-end" for dplyr. - Basically, write your code as if you were using dplyr and then it gets automatically translated to (and evaluated as) data.table code.
:::
